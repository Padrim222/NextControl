...
export default function CSInbox() {
    ...
    const handleApprove = async (id: string) => {
        try {
            ...
            // Trigger Email Notification (Pocket Trainer Action)
            try {
                const { error: funcError } = await supabase.functions.invoke('notify-client', {
                    body: { submission_id: id }
                });
                if (funcError) console.warn('Falha ao enviar notificação:', funcError);
                else toast.info('Cliente notificado por email! 🚀');
            } catch (e) {
                console.warn('Erro ao chamar notificação:', e);
            }
